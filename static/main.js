...
async function fetchAndUpdateMapData() {
  try {
    ...
    const STATUS_INTERVAL_MS = 15 * 60 * 1000; // 15 minutes between "still in/out" status updates per beacon
    ...
    aggBeacons.forEach(b => {
      ...
      if (prev !== nowState) {
        ...
        // Beacon stayed in the same state: send a gentle "still in/out"
        // status update occasionally, but keep it local to avoid DB spam.
        const lastStatusAt = beaconLastStatusAt[key] || 0;
        if (nowMs - lastStatusAt >= STATUS_INTERVAL_MS) {
          if (nowState === 'in') {
            addNotification(
              'status',
              `${b.name || b.id} is still in range`,
              b.last_seen,
              dist
            );
          } else {
            addNotification(
              'status',
              `${b.name || b.id} is still out of range`,
              b.last_seen,
              dist
            );
          }
    ...
    goodDevices.forEach(d => {
      ...
      if (!wasOffline && isOffline) {
        ...
        addNotification('offline', label, d.timestamp, null);
      } else if (wasOffline && !isOffline) {
        deviceStatus[ident] = 'online';
        addNotification('online', label, d.timestamp, null);
      }
    });
