...
def compute_device_connectivity_summary(start_iso: str, end_iso: str, device_ident: str | None = None) -> dict:
    """Return offline/online intervals and totals for devices within [start_iso, end_iso] using notifications.created_at.

    - Uses notifications.type in ('offline','online')
    - Pairs OFFLINE -> ONLINE to form downtime intervals
    - If a device is still offline at end_iso, interval is closed at end_iso
    """
    ...
    conn = get_db()
    try:
        # Best-effort: ensure devices table exists so we can resolve friendly names
        conn.execute(
            """
        ...
        # Fetch device name map
        dev_rows = conn.execute("SELECT id, name FROM devices").fetchall()
        dev_name = {did: (nm or did) for (did, nm) in dev_rows if did}

        q = (
            "SELECT type, created_at, beacon_name, device_ident "
            "FROM notifications "
            "WHERE type IN ('offline','online') "
            "AND created_at >= ? AND created_at <= ?"
        )
        params = [start_iso, end_iso]
        if device_ident:
            q += " AND device_ident = ?"
            params.append(device_ident)
        q += " ORDER BY device_ident, created_at"

        rows = conn.execute(q, params).fetchall()

        intervals_by_device: dict[str, list[dict]] = {}
        total_downtime_s_by_device: dict[str, int] = {}

        end_dt = _parse(end_iso)
        # Track open offline interval per device
        open_offline: dict[str, str] = {}

        for typ, created_at, name, did in rows:
            did = did or ""
            if not did:
                # If device_ident isn't stored, fall back to name as key
                did = name or "UNKNOWN_DEVICE"
            if did not in intervals_by_device:
                intervals_by_device[did] = []
                total_downtime_s_by_device[did] = 0

            if typ == "offline":
                # Start offline if none open
                if did not in open_offline:
                    open_offline[did] = created_at
            elif typ == "online":
                # Close offline interval if open
                if did in open_offline:
                    start_ts = open_offline.pop(did)
                    sdt = _parse(start_ts)
                    edt = _parse(created_at)
                    if sdt and edt and edt >= sdt:
                        dur_s = int((edt - sdt).total_seconds())
                    else:
                        dur_s = 0
                    intervals_by_device[did].append(
                        {
                            "offline_at": start_ts,
                            "online_at": created_at,
                            "duration_s": dur_s,
                            "duration_label": _dur_label(dur_s),
                        }
                    )
                    total_downtime_s_by_device[did] += dur_s

        # Close any still-offline intervals at end_iso
        for did, start_ts in list(open_offline.items()):
            sdt = _parse(start_ts)
            if sdt and end_dt and end_dt >= sdt:
                dur_s = int((end_dt - sdt).total_seconds())
            else:
                dur_s = 0
            intervals_by_device.setdefault(did, []).append(
                {
                    "offline_at": start_ts,
                    "online_at": end_iso,
                    "duration_s": dur_s,
            ...
            total_downtime_s_by_device[did] = total_downtime_s_by_device.get(did, 0) + dur_s

        # Build a simple flattened list for rendering
        rendered = []
        for did, intervals in intervals_by_device.items():
            if not intervals:
                continue
            label = dev_name.get(did, did)
            rendered.append(
                {
                    "device_ident": did,
                    "device_label": label,
                    "intervals": intervals,
                    "total_downtime_s": total_downtime_s_by_device.get(did, 0),
                }
            )

        rendered.sort(key=lambda x: x["device_label"].lower())

        total_intervals = sum(len(d["intervals"]) for d in rendered)
        total_downtime_s = sum(d["total_downtime_s"] for d in rendered)

        return {
            "start": start_iso,
            "end": end_iso,
            "devices": rendered,  # list of {device_label, intervals, total_downtime_s}
...
def generate_report_pdf(report_entries, created_at_iso, pdf_path, device_connectivity=None):
    """
    ...
    # Optional: device connectivity section (offline/online)
    if device_connectivity is not None:
        c.showPage()
        y = height - margin
        c.setFont("Helvetica-Bold", 14)
        c.drawString(margin, y, "Device Connectivity")
        y -= 18
        c.setFont("Helvetica", 10)
        if not device_connectivity.get("devices"):
            c.drawString(margin, y, "No device offline/online events recorded in this period.")
            y -= 14
        else:
            c.setFont("Helvetica", 9)
            for dev in device_connectivity["devices"]:
                if y < 80:
                    c.showPage()
                    y = height - margin
                    c.setFont("Helvetica-Bold", 14)
                    c.drawString(margin, y, "Device Connectivity (cont.)")
                    y -= 18
                    c.setFont("Helvetica", 9)
                label = dev.get("device_label") or dev.get("device_ident") or "Device"
                total_s = int(dev.get("total_downtime_s") or 0)
                total_m = total_s // 60
                header = f"{label} — total downtime ~{total_m} min" if total_m else f"{label}"
                c.setFont("Helvetica-Bold", 10)
                c.drawString(margin, y, header)
                y -= 12
                c.setFont("Helvetica", 9)
                for itv in dev.get("intervals") or []:
                    if y < 60:
                        c.showPage()
                        y = height - margin
                        c.setFont("Helvetica-Bold", 14)
                        ...
                        y -= 18
                        c.setFont("Helvetica", 9)
                    off_at = (itv.get("offline_at") or "-").replace("T", " ")
                    on_at = (itv.get("online_at") or "-").replace("T", " ")
                    dur = itv.get("duration_label") or "-"
                    note = " (still offline)" if itv.get("open_ended") else ""
                    c.drawString(margin + 12, y, f"Offline {off_at} → Online {on_at}  ({dur}){note}")
                    y -= 11
                y -= 6
    ...
    if device_connectivity is not None:
        tot_int = int(device_connectivity.get("total_intervals") or 0)
        tot_s = int(device_connectivity.get("total_downtime_s") or 0)
        tot_m = tot_s // 60
        if tot_int:
            summary_text += f"; {tot_int} device downtime intervals (~{tot_m} min)"
    return summary_text
...
def generate_daily_report():
    ...
    # Device connectivity for the report day (offline/online events)
    day = (created_at_iso or "").split("T")[0]
    if day:
        start_iso = f"{day}T00:00:00"
        device_connectivity = compute_device_connectivity_summary(start_iso, created_at_iso)
    else:
        device_connectivity = None

    summary_text = generate_report_pdf(report, created_at_iso, pdf_path, device_connectivity=device_connectivity)
...
def generate_device_activity_report(device_ident, start_date=None, end_date=None):
    ...
    # Build a short text summary for history tables
    summary = line1 + " | " + line2
    if presence:
        summary += f" | In~{presence['in_percent']}% Out~{presence['out_percent']}%"
    ...
    # Device connectivity (offline/online) within the selected window (uses notifications.created_at)
    ...
    if start_iso and not end_iso:
        end_iso = samoa_iso_now()
    if end_iso and not start_iso:
        # If only end date provided, look back to start of that day
        start_iso = f"{end_date}T00:00:00"
    if not start_iso and not end_iso:
        # Default window: today
        day = samoa_iso_now().split("T")[0]
        start_iso = f"{day}T00:00:00"
        end_iso = samoa_iso_now()

    conn_info = compute_device_connectivity_summary(start_iso, end_iso, device_ident=device_ident)

    c.setFont("Helvetica-Bold", 11)
    c.drawString(margin, y, "Device Connectivity")
    y -= 14
    c.setFont("Helvetica", 9)
    if not conn_info.get("devices"):
        c.drawString(margin + 10, y, "No OFFLINE/ONLINE events recorded in this period.")
        y -= 14
    else:
        # Only one device expected here, but keep generic
        for devinfo in conn_info["devices"]:
            for itv in devinfo.get("intervals") or []:
                if y < 70:
                    c.showPage()
                    y = height - margin
                    c.setFont("Helvetica-Bold", 16)
                    c.drawString(margin, y, "Device Activity Report (cont.)")
                    y -= 24
                    c.setFont("Helvetica-Bold", 11)
                    c.drawString(margin, y, "Device Connectivity")
                    y -= 14
                ...
                dur = itv.get("duration_label") or "-"
                note = " (still offline)" if itv.get("open_ended") else ""
                c.drawString(margin + 10, y, f"Offline {off_at} → Online {on_at}  ({dur}){note}")
                y -= 12
        y -= 4
