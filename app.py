...
def save_notification():
    ...
    if ntype in BEACON_TYPES:
        ...
        if ntype in ("in", "left"):
            ...
            if row:
                conn.execute(
                    "UPDATE beacon_states SET state = ?, last_change_ts = ?, last_still_ts = ? WHERE beacon_key = ?",
                    (desired_state, now_ts, now_ts, beacon_key),
                )
            else:
                conn.execute(
                    "INSERT INTO beacon_states (beacon_key, state, last_change_ts, last_still_ts) VALUES (?, ?, ?, ?)",
                    (beacon_key, desired_state, now_ts, now_ts),
                )
