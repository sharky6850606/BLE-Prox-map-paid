...
def _fallback_db_path() -> str:
    """If the configured DB path is not writable/healthy (eg Render disk hiccup),
    fall back to a tmp location so the app can still boot.
    
    Note: falling back means data may not persist across restarts, but it's
    better than crashing with 500s everywhere.
    """
    tmp_root = os.environ.get("PROXMAP_TMP_DATA", "/tmp/proxmap_data")
    return os.path.join(tmp_root, "beacons.db")
...
def get_db() -> sqlite3.Connection:
    ...
    try:
        ...
        # e.g. "disk I/O error" or permission issues
    ...
    try:
        ...
        pass
    # Quick health check: if the underlying storage is broken, this will raise
    # and we'll retry in tmp.
    try:
